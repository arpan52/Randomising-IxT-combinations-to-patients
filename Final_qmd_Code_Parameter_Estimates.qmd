---
title: "Code for obtaining the fixed and random effect estimates and Type-2 error"
format: html
editor: visual
---
...
Analysis_final <- function(Design,Example,del_0,del_1,del_2,del_3) {
  ...
  if (Design==1){
    ...
    if (Example==1){
      ...
      Design_num<-"Design D11 - Completely Randomised Parallel-Group Design - 
            non-randomly assign therapists to patients after randomising interventions to patients"  
      
      
      df <- intervention_generation(data=df, n_Int=2, Int_name = "Int")
      
      # Ensure Int is numeric for matrix operations
      df$Int_numeric <- as.numeric(as.character(df$Int))
      # Add a column of interactions
      df$IT <- interaction(df$Int, df$Therapist)
      # baseline patient Age for bias
      patient_avAge <- del_3 + rnorm(max(df$Int_numeric)*length(levels(df$Therapist)))
      # random patient Age  
      patAge <- patient_avAge[df$IT] +  rnorm(n_Patient) 
  ...
  if (Design==1){
    ...
    if (Example==2){
      ...
      Design_num<- "Design D12- Randomised Block [Batch] Parallel-Group Design - 
               non-randomly assign therapists to patients after randomising interventions to patients"   
  ...
  if (Design==1){
    ...
    if (Example==3){
      ...
      Design_num<- "Design D13 - Randomised Block [Batch, Centre] Parallel-Group 
                                Design - non-randomly assign therapists to patients after randomising interventions to patients"   
  ...
  if (Design==2){
    ...
    if (Example==1){
      ...
      Design_num <- "Design D21 Completely Randomised Parallel-Group Design -  
            non-randomly assign therapists to patients before randomising interventions to patients"
  ...
  if (Design==2){
    ...
    if (Example==2){
      ...
      Design_num<- "Design D22 - Randomised Block [Batch] Parallel-Group Design -
            non-randomly assign therapists to patients before randomising interventions to patients"   
  ...
  if (Design==2){
    ...
    if (Example==3){
      ...
      Design_num<- "Design D23 - Randomised Block [Batch, Centre] Parallel-Group  
                        Design - non-randomly assign therapists to patients before randomising interventions to patients"   
  ...
  if (Design==5){
    ...
    if (Example==1){
      ...
      Design_num<-"Design D51 - Completely Randomised Factorial Design - randomising interventions to patients, then randomising therapists to patients, blocking the latter by the randomised intervention"  
     ...
     # View(ds_ordered)
      #print(sum(ds$Therapist == 2 & ds$Int == 1))
  ...
  if (Design==5){
    ...
    if (Example==2){
      ...
      Design_num<- "Design D52- Randomised Block Factorial Design - randomising interventions to patients, then randomising therapists to patients, blocking the latter by the randomised intervention"    
  ...
  if (Design==5){
    ...
    if (Example==3){
      ...
      Design_num<- "Design D53 - Multicentre Randomised Block Factorial Design - 
            randomising interventions to patients, then randomising therapists to patients, blocking the latter by the randomised intervention"  
  ...
  if(exists("data_structure")) {
    ...
    #print(sum1)
    E_cont<-rep(0,5)
    # Access the estimate for 'contrast' from the coefficient table
    E_cont[1] <-  as.numeric(sum1$coefficients["contrast", "Estimate"])
    E_cont[2] <-  as.numeric(sum1$coefficients["contrast", "Std. Error"])
    E_cont[3] <- as.numeric(attr(VarCorr(reg)[["Therapist"]], "stddev")[1])
    E_cont[4] <- as.numeric(attr(VarCorr(reg)[["contrast:Therapist"]], "stddev")[1])
    
    # degree of freedom of contrast
    contrast_df <- as.numeric(sum1$coefficients["contrast", "df"])
    ...
    if(p_value_contrast > 0.05){E_cont[5] <- 1}
    if(p_value_contrast < 0.05){E_cont[5] <- 0}
...
The next code make use of the function Analysis_final for n_sim number of simulations and provides the averages of "estimate of Contrast, its standard error, ICC of the Therapist effect and Therapist x Intervention combinations, Type-I and II errors" as output.
...
Comarison_D_ij <- function(Design,Example,del_0,del_1,del_2,del_3,n_sim) {
  ...
  m<-rep(0, length(del_2))
  s<-rep(0, length(del_2))
  t<-rep(0, length(del_2))
  it<-rep(0, length(del_2))
  t2_er<-rep(0, length(del_2))
  ...
  for (i in del_2) {
    ...
    for (j in 1:n_sim) {
      ...
      # compute SE of the estimate over n_sim simulations
      
      out<- Analysis_final(Design,Example,del_0,del_1,i,del_3)
      Cont[j]<- as.numeric(out[1])
      Std[j]<-  as.numeric(out[2])
      Th[j]  <-  as.numeric(out[3])
      IT[j] <-  as.numeric(out[4])
      contr_2<- contr_2+as.numeric(out[5])
      
    }
    # print the design type
    print(out[6]) 
    
    m[k]<-mean(Cont)   # Compute the avarage of the estimate of contrast  of second scenario
    s[k]<-mean(Std)    # Compute the avarage of the SE estimate of contrast  of second scenario
    t[k]<-mean(Th)      # Compute the avarage of the SD of the Therapist effect of second scenario
    it[k]<-mean(IT)   # Compute the avarage of the SD of the Therapist by Intervention effect of second scenario
    t2_er[k]<- contr_2/n_sim 
  ...
  return(round(cbind(m,s,t^2,it^2,t2_er),3))
}
```
...
C_interleaved <- C[, idx]
# custom colomn names
colnames(C_interleaved) <- c(  paste("delta_1 D", Design_1, Example_1),
